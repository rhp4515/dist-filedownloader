...
func (cmd commandLoc) RequireParam() bool {
	return true
}

func (cmd commandLoc) RequireAuth() bool {
	return true
}

func (cmd commandLoc) Execute(conn *ftpConn, param string) {
	path := conn.buildPath(param)
	data := conn.driver.LocateFile(path)
	if data != "-1" {
